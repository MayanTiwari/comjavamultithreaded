...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    private boolean check() {
        if (!isBST())            out.println("Not in symmetric order");
        if (!isSizeConsistent()) out.println("Subtree counts not consistent");
        if (!isRankConsistent()) out.println("Ranks not consistent");
        if (!is23())             out.println("Not a 2-3 tree");
        if (!isBalanced())       out.println("Not balanced");
